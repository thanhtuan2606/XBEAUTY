    ...
    if (isset($_POST["resetpass_submit"])) {
        ...
        if ($result) {
            ...
            try {
                ...
                $mail->setFrom('hancr04@gmail.com', 'XBEAUTY');
                ...
                $mail->Body = 'Mật khẩu tạm thời: ' . $new_password . '<br>
                    ...
                    <p style="color: red; font-size: 14px;">XBEAUTY chân thành cảm ơn.</p>';
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["token"])) {
    ...
    // Lấy thông tin token và thời gian hết hạn từ cơ sở dữ liệu
    $query_token = "SELECT token_expiry FROM khachhang WHERE reset_token = :token";
    $stmt_token = $conn->prepare($query_token);
    $stmt_token->bindParam(':token', $token);
    $stmt_token->execute();
    $token_info = $stmt_token->fetch(PDO::FETCH_ASSOC);
